...
public class RecurrenceSet {
	private final static String TAG = "KRecurrenceSet";
        ...
        for (int i = 0; i<n; ++i) {
            ...
            try {
            	try {
					...
					Date untilDate = fmt.parse(rawDates[i]);
					time.setTimeZone(timezone);                	
					time.setTime(untilDate);//time.parse(rawDates[i]);
            ...
            dates[i] = time.getTimeInMillis();
            time.setTimeZone(timezone);//time.timezone = tz;
    ...
    public static boolean populateContentValues(ICalendar.Component component,
        ...
        try {
            ...
            TimeZone timezone;
            if (tzidParam == null) {
            	tzid = ETime.TIMEZONE_UTC;
            	timezone = TimeZone.getTimeZone(ETime.TIMEZONE_UTC);
            }
            else {
            	timezone = TimeZone.getTimeZone(tzid);
            }
            
            Calendar start = GregorianCalendar.getInstance(timezone);//Time start = new Time(tzidParam == null ? Time.TIMEZONE_UTC : tzid);
                   
            try {
                //TimeZone utcTimezone = TimeZone.getTimeZone(ETime.TIMEZONE_UTC);
                SimpleDateFormat fmt = new SimpleDateFormat(ETime.ISO_8601_BASIC_DATETIME_NOMILLIS_UTC_TZ_FORMAT);
				fmt.setTimeZone(timezone);
				Date dtstartDate = fmt.parse(dtstart);
				start.setTimeZone(timezone);                	
				start.setTime(dtstartDate);                
            } catch (ParseException e) {
            	e.printStackTrace();
            }               
            
            // 아래 과정이 넘 웃기다...
            // 이미 dtstartProperty에서 timezone id를 식별해서
            // 위에서 utc인지 아닌지를 판가름하고 있는데...
            //boolean inUtc = start.parse(dtstart);
            boolean inUtc = false;
            if (tzid.compareTo(ETime.TIMEZONE_UTC) == 0) {
            ...
            if (allDay) {
                start.setTimeZone(TimeZone.getTimeZone(ETime.TIMEZONE_UTC));
            }
	...
	*/
    ...
    public static boolean populateComponent(Cursor cursor,
        ...
        Calendar dtstartTime = null;
        if (!TextUtils.isEmpty(tzid)) {
            if (!allDay) {
                dtstartProp.addParameter(new ICalendar.Parameter("TZID", tzid));
            }
            dtstartTime = GregorianCalendar.getInstance(TimeZone.getTimeZone(tzid));
        } else {
            // use the "floating" timezone
            dtstartTime = GregorianCalendar.getInstance(TimeZone.getTimeZone(ETime.TIMEZONE_UTC));//new Time(Time.TIMEZONE_UTC);
        }

        dtstartTime.setTimeInMillis(dtstart);
    ...
    */
    ...
    /*
    private static String computeDuration(Calendar start,
                                          ICalendar.Component component) {
