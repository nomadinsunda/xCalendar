...
import com.intheeast.calendarcommon.RecurrenceProcessor;
import com.intheeast.calendarcommon.RecurrenceSet;
...
public class EditEventHelper {
    ...
    private com.intheeast.calendarcommon2.EventRecurrence mEventRecurrence = new com.intheeast.calendarcommon2.EventRecurrence();

    public static final String NO_EVENT_COLOR = "";
    ...
    public static class EventInfoMainFrameAttendeeItem {
    	//public boolean mRemoved;
        public Attendee mAttendee; 
        //public int mUpdateCounts;
        //public Uri mContactLookupUri;
        ...
        public EventInfoMainFrameAttendeeItem(Attendee attendee) {
            mAttendee = attendee;            
        }
    }
    ...
    public boolean saveEvent(CalendarEventModel model, CalendarEventModel originalModel,
        ...
        // mOwnerAttendeeId�� cursor.getInt(EditEventHelper.ATTENDEES_INDEX_ID);
        // mOwnerAttendeeId�� -1�� �Ǵ°���?
        // :-new event�� ���
        //  -������ event�� load�Ͽ��� �� ���� event�� attendee���� �����ϰ�
        //   mModel.mOwnerAccount�� attendee�� �������� email�� �ϳ��� �������� ������ �ʱ⃔ -1�� ������ 
        //   (�����ϸ� -1�� �ƴ϶� Attendees._ID�� ������)
        if (hasAttendeeData && model.mOwnerAttendeeId == -1) {
            // Organizer is not an attendee

        	// mOwnerAccount = cursor.getString(EVENT_INDEX_OWNER_ACCOUNT);
    ...
    protected long constructDefaultStartTime(long now) {
        Calendar defaultStart = GregorianCalendar.getInstance();//new Time();
        defaultStart.setTimeInMillis(now);
        defaultStart.set(Calendar.SECOND, 0);//defaultStart.second = 0;
        defaultStart.set(Calendar.MINUTE, 30);//defaultStart.minute = 30;
        long defaultStartMillis = defaultStart.getTimeInMillis();
    ...
    void checkTimeDependentFields(CalendarEventModel originalModel, CalendarEventModel model,
        ...
        if (modifyWhich == MODIFY_ALL) {
            ...
            if (newAllDay) {
                Calendar time = GregorianCalendar.getInstance(TimeZone.getTimeZone(ETime.TIMEZONE_UTC));//new Time(Time.TIMEZONE_UTC);
                time.setTimeInMillis(oldStartMillis);
                ...
                oldStartMillis = time.getTimeInMillis();
            }
    ...
    public String updatePastEvents(ArrayList<ContentProviderOperation> ops,
        ...
        Calendar dtstart = GregorianCalendar.getInstance(TimeZone.getTimeZone(originalModel.mTimezone));//Time dtstart = new Time(); dtstart.timezone = originalModel.mTimezone;
        dtstart.setTimeInMillis(startTimeMillis);
        ...
        if (origRecurrence.count > 0) {
            ...
            Calendar untilTime = GregorianCalendar.getInstance(TimeZone.getTimeZone(ETime.TIMEZONE_UTC));//Time untilTime = new Time();
            //untilTime.timezone = Time.TIMEZONE_UTC;

            // Subtract one second from the old begin time to get the new
            // "until" time.
            untilTime.setTimeInMillis(endTimeMillis - 1000); // subtract one second (1000 millis)
            if (origAllDay) {
                untilTime.set(Calendar.HOUR_OF_DAY, 0);//untilTime.hour = 0;
                untilTime.set(Calendar.MINUTE, 0);//untilTime.minute = 0;
                untilTime.set(Calendar.SECOND, 0);//untilTime.second = 0;
                ...
                dtstart.set(Calendar.HOUR_OF_DAY, 0);//dtstart.hour = 0;
                dtstart.set(Calendar.MINUTE, 0);//dtstart.minute = 0;
                dtstart.set(Calendar.SECOND, 0);//dtstart.second = 0;
                //dtstart.allDay = true;
                dtstart = ETime.switchTimezone(dtstart, TimeZone.getTimeZone(ETime.TIMEZONE_UTC));//dtstart.timezone = Time.TIMEZONE_UTC;
            }
            origRecurrence.until = ETime.format2445(untilTime);//origRecurrence.until = untilTime.format2445();
        }

        updateValues.put(Events.RRULE, origRecurrence.toString());
        updateValues.put(Events.DTSTART, dtstart.getTimeInMillis());
        ContentProviderOperation.Builder b =
    ...
    void addRecurrenceRule(ContentValues values, CalendarEventModel model) {
        ...
        // mEnd and mStart in UTC millis since epoch
        long end = model.mEnd;
    ...
    static void updateRecurrenceRule(int selection, CalendarEventModel model,
        ...
        if (selection == DOES_NOT_REPEAT) {
            ...
            Calendar startTime = GregorianCalendar.getInstance(TimeZone.getTimeZone(model.mTimezone));//Time startTime = new Time(model.mTimezone);
            startTime.setTimeInMillis(model.mStart);//startTime.set(model.mStart);
            

            days[0] = EventRecurrence.calendarDay2Day(startTime.get(Calendar.DAY_OF_WEEK));
            ...
            Calendar startTime = GregorianCalendar.getInstance(TimeZone.getTimeZone(model.mTimezone));//Time startTime = new Time(model.mTimezone);
            startTime.setTimeInMillis(model.mStart);//startTime.set(model.mStart);
            bymonthday[0] = startTime.get(Calendar.DAY_OF_MONTH);
            ...
            int dayCount = 1 + ((startTime.get(Calendar.DAY_OF_WEEK) - 1) / 7);
            if (dayCount == 5) {
                dayCount = -1;
            }
            bydayNum[0] = dayCount;
            byday[0] = EventRecurrence.calendarDay2Day(startTime.get(Calendar.DAY_OF_WEEK));
            eventRecurrence.byday = byday;
            eventRecurrence.bydayNum = bydayNum;
    ...
    ContentValues getContentValuesFromModel(CalendarEventModel model) {
        ...
        Calendar startTime = GregorianCalendar.getInstance(TimeZone.getTimeZone(timezone));//Time startTime = new Time(timezone);
        Calendar endTime = GregorianCalendar.getInstance(TimeZone.getTimeZone(timezone));//Time endTime = new Time(timezone);

        startTime.setTimeInMillis(model.mStart);
        endTime.setTimeInMillis(model.mEnd);
        ...
        if (isAllDay) {
            ...
            timezone = ETime.TIMEZONE_UTC;
            startTime.set(Calendar.HOUR_OF_DAY, 0);//startTime.hour = 0;
            startTime.set(Calendar.MINUTE, 0);//startTime.minute = 0;
            startTime.set(Calendar.SECOND, 0);//startTime.second = 0;
            startTime = ETime.switchTimezone(startTime, TimeZone.getTimeZone(timezone));//startTime.timezone = timezone;
            startMillis = startTime.getTimeInMillis();//startTime.normalize(true);

            endTime.set(Calendar.HOUR_OF_DAY, 0);//endTime.hour = 0;
            endTime.set(Calendar.MINUTE, 0);//endTime.minute = 0;
            endTime.set(Calendar.SECOND, 0);//endTime.second = 0;
            endTime = ETime.switchTimezone(endTime, TimeZone.getTimeZone(timezone));//endTime.timezone = timezone;
            endMillis = endTime.getTimeInMillis();//endTime.normalize(true);
     ...
     */
    private void offsetStartTimeIfNecessary(Calendar startTime, Calendar endTime, String rrule,
        ...
        int weekstart = EventRecurrence.day2CalendarDay(mEventRecurrence.wkst);
        int startDay = startTime.get(Calendar.DAY_OF_WEEK);
        for (int i = 0; i < mEventRecurrence.bydayCount; i++) {
            int day = EventRecurrence.day2CalendarDay(mEventRecurrence.byday[i]);
            if (day == startDay) {
        ...
        startTime.add(Calendar.DAY_OF_MONTH, daysOffset);//startTime.monthDay += daysOffset;
        endTime.add(Calendar.DAY_OF_MONTH, daysOffset);//endTime.monthDay += daysOffset;
        long newStartTime = startTime.getTimeInMillis();
        long newEndTime = endTime.getTimeInMillis();
