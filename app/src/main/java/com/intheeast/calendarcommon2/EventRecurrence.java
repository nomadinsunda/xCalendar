package com.intheeast.calendarcommon2;
...
public class EventRecurrence {
    ...
    public Calendar      startDate;     // set by setStartDate(), not parse()
    ...
    public void setStartDate(Calendar date) {
        startDate = GregorianCalendar.getInstance(date.getTimeZone());
	...
	*/
    ...
    public boolean equals(Object obj) {
        ...
        return  (startDate == null ?
                        er.startDate == null : ETime.compare(startDate, er.startDate) == 0) &&
                freq == er.freq &&
    ...
    // 아이캘린더(iCalendar)는 인터넷 사용자들이 다른 인터넷 사용자들에게 전자 메일을 이용하여 미팅 요청과 할 일을 보내거나 .ics 확장자로 파일들을 공유할 수 있게 해 주는 컴퓨터 파일 형식이다. 
    // 아이캘린더 데이터 파일을 받은 사람들은 이메일 클라이언트나 캘린더 프로그램 따위를 이용하여 보낸이에게 쉽게 응답할 수 있고 다른 미팅 일정을 역제안할 수 있다
    //
    // -UNTIL rule part는 반복 규칙의 종료일을 결정한다. 
    //  UNTIL의 value type은 "DTSTART" 의 value type과 똑같아야 한다. 
    //  "DTSTART" 가 local time 형식을 사용했다면 UNTIL도 local time 형식을 사용해야 한다. 
    //  "DTSTART"가 UTC를 사용했다면 UNTIL 도 UTC를 사용해야 한다.  
    //  "STANDARD"와 "DAYLIGHT" 같은 서브-컴포넌트를 사용할 경우에는 UNTIL은 반드시 UTC시간을 사용해야 한다.  
    //   DATE-TIME value를 사용했다면, 반드시 UTC 시간형식으로 사용해야 한다. 
    //   만약 UNTIL을 지정하지 않고 COUNT rule part도 지정하지 않았다면 "RRULE" 은 계속 반복되게 된다.
    //
    // -COUNT rule part 는 반복 일정안에서 몇번 반복될지 지정한다. 이 때 "DTSTART" 에 한번 발생한것으로 카운트 한다.
    //
    // -INTERVAL part
    //  :양수 값으로써 반복 규칙이 어떤 간격으로 반복되는지 설정한다. 
    //   기본값은 "1"이다. 
    //   기본값 1일 때 각각의 의미는 다음과 같다
    //   SECONDLY rule에서는 매초마다 라는 의미이다. 
    //   MINUTELY rule에서는 매분마다 라는 의미이다. 
    //   HOURLY rule에서는 매시간마다 라는 의미이다. 
    //   DAILY rule에서는 매일마다 라는 의미이다. 
    //   WEEKLY rule에서는 매주마다 라는 의미이다. 
    //   MONTHLY rule에서는 매월마다 라는 의미이다. Y
    //   YEARLY rule에서는 매년마다 라는 의미이다. 
    //   (예 DAILY rule INTERVAL=8 이면 매8일 마다 라는 의미이다.)
    //
    ...
    public void parse(String recur) {
         ...
         *
         *       ( ";" "INTERVAL" "=" 1*DIGIT )          /
         *        ->FREQ=HOURLY;INTERVAL=4 ->매 4시간 간격
         *          FREQ=MINUTELY;INTERVAL=15 ->매 15분의 간격
         *          
         *       ( ";" "BYSECOND" "=" byseclist : 0 ~ 60 [COMMA로 구분된 값 여러개가 리스트로 들어갈 수 있다] )        /
         *       ( ";" "BYMINUTE" "=" byminlist : 0 ~ 59 [COMMA로 구분된 값 여러개가 리스트로 들어갈 수 있다] )        /
         *       ( ";" "BYHOUR" "=" byhrlist : 0 ~ 23 [COMMA로 구분된 값 여러개가 리스트로 들어갈 수 있다] )           /
         *        ->BYSECOND 와 BYMINUTE 그리고 BYHOUR 는 "DTSTART"가 DATE value type일 때는 사용할 수 없다. 만약에 있다면 무시해야 한다
         *        
         *       ( ";" "BYDAY" "=" bywdaylist : SU 일요일, MO 월요일, TU 화요일, WE 수요일, TH 목요일, FR 금요일, SA 토요일 [COMMA로 구분된 값 여러개가 리스트로 들어갈 수 있다] )          /
         *        ->각 BYDAY의 값 앞에는 양수(+n)이나 음수(-n)이 들어갈 수 있다. 
         *          이 숫자는 몇번째에 반복되는지를 의미한다. 
         *          예를들어
         *          MONTHLY rule일 때 +1MO (또는 1MO) 는 해당 달의 첫번째 월요일을 의미하고 -1MO 는 해당 달의 마지막 월요일을 의미한다. 
         *          FREQ가 YEARLY이고 BYMONTH rule part가 있을때에는 해당 달 안에서 몇번째 인지를 의미한다.  
         *          만약 요일 앞에 숫자가 없으면 해당하는 요일이 모두 반복된다. 예를 들어 MONTHLY rule에서 MO는 그 달의 모든 월요일을 의미한다. 
         *          숫자는  FREQ가 MONTHLY 나 YEARLY 일 때만 넣을 수 있다. 
         *          그 외에는 숫자를 넣을 수 없다. 
         *          그리고 FREQ가 YEARLY 이더라도 BYWEEKNO rule part가 지정되어 있으면 숫자를 넣을 수 없다.
         *          
         *       ( ";" "BYMONTHDAY" "=" bymodaylist : 1 ~ 31 또는 -31 ~ -1 [COMMA로 구분된 값 여러개가 리스트로 들어갈 수 있다] )    /
         *        ->-10 은 그 달의 뒤에서 10번째 일자이다. FREQ가 WEEKLY 이면 BYMONTHDAY를 지정할 수 없다.
         *        
         *       ( ";" "BYYEARDAY" "=" byyrdaylist : 1 ~ 366 or -366 ~ -1[COMMA로 구분된 값 여러개가 리스트로 들어갈 수 있다] )     /
         *       
         *       ( ";" "BYWEEKNO" "=" bywknolist : 1 ~ 53 or -53 ~ -1 [COMMA로 구분된 값 여러개가 리스트로 들어갈 수 있다] )       /
         *        ->[ISO.8601.2004] 에 정의된 week 번호를 따른다. 
         *          한 주는 7일 간격으로 되어 있고 한 주의 시작 요일은 WKST로 지정된다. 
         *          (기본값은 월요일이다.) 한 해의 첫번째 주는 최소 4일을 포함하고 있어야 한다.  
         *          BYWEEKNO는 FREQ가 YEARLY 일 때만 사용해야 한다.  
         *          BYWEEKNO가 3이면 그 해의 3번째 주라는 의미이다.
         *          
         *       ( ";" "BYMONTH" "=" bymolist : 1 ~ 12 [COMMA로 구분된 값 여러개가 리스트로 들어갈 수 있다] )          /
         *       
         *       ( ";" "BYSETPOS" "=" bysplist : 1~366 or -366~-1 [COMMA로 구분된 값 여러개가 리스트로 들어갈 수 있다] )         /
         *        ->이 값은 반복되는 범위 안에서 몇 번째에 발송하는지를 지정하는 규칙이다. 값은 1~366 또는 -366~-1 까지 들어갈 수 있다. 
         *          이 것은 반드시 BYxxx rule part와 같이 쓰여야 한다. 예를들어서 다음과 같이 쓴다: 
		 *          FREQ=MONTHLY;BYDAY=MO,TU,WE,TH,FR;BYSETPOS=-1
		 *			"매월 마지막 주 월~금" 라는 의미.
		 *
         *       ( ";" "WKST" "=" weekday : MO,TU,WE,TH,FR,SA,SU )              /
         *        ->이 값은 RRULE의 interval이 1보다 크고 BYDAY rule을 사용할 때 중요하다. 
         *          또 YEARLY rule에 BYWEEKNO rule part를 사용할 때 도 중요하다. 기본값은 MO 이다.
         *          
         *       ( ";" x-name "=" text )
         *       
         *       )
         *       
         *  *rule에서 시간정보가 없으면 시작시간("DTSTART") 의 날짜와 시간을 그대로 사용한다. 
         *  예를 들어서 "FREQ=YEARLY;BYMONTH=1" 이라는 것이 있을 때 여기에는 일자와 시간정보가 없는데 이때 "DTSTART"에서 일자와 시간정보를 그대로 가져다 쓴다.
         *
         *  *BYxxx rule part 는 특정한 규칙에 따라 반복규칙을 조절하게 된다. BYxxx rule의 시간범위가 FREQ에 설정한 규칙의 시간범위와 같거나 클경우에는 보통 반복횟수를 줄이게 된다. 
         *  예를 들면 "FREQ=DAILY;BYMONTH=1" 의 경우에 BYMONTH가 없었으면 매일매일 반복되었을 것인데 BYMONTH=1이 들어감으로써 1월달만 반복되는 것으로 제한되게 된다. 
         *  비슷하게 BYxxx의 시간범위가 FREQ에 설정한 규칙의 시간범위보다 작을 때에는 보통 반복횟수가 늘어나게 된다. 
         *  예를들면 "FREQ=YEARLY;BYMONTH=1,2" 가 있다면 1년에 한번 있을 일정이 2번으로 늘어나게 된다.
         *  
         *  *만약 여러개의 BYxxx rule part가 사용된다면, 먼저 FREQ와 INTERVAL rule 부터 평가를 하고 난 다음에 다음 순서대로 평가한다:  
         *  BYMONTH, BYWEEKNO, BYYEARDAY, BYMONTHDAY, BYDAY, BYHOUR, BYMINUTE, BYSECOND, BYSETPOS; 그런 후 COUNT 와 UNTIL 이 평가된다.
         *  
         *  *다음 rrule을 해석해 보면
         *  DTSTART;TZID=America/New_York:19970105T083000 
         *  RRULE:FREQ=YEARLY;INTERVAL=2;BYMONTH=1;BYDAY=SU;BYHOUR=8,9;BYMINUTE=30
         *  "FREQ=YEARLY"   - 매년 
         *  "INTERVAL=2"    - 격년(1년건너 한번) 
         *  "BYMONTH=1"     - 격년, 매1월 
         *  "BYDAY=SU"      - 격년, 매1월, 매주 일요일 
         *  "BYHOUR=8,9"    - 격년, 매1월, 매주 일요일 8 AM과  9 AM 
         *  "BYMINUTE=30"   - 격년, 매1월, 매주 일요일 8:30 AM과  9:30 AM 없는 정보인 초(second)정보는 DTSTART에서 가져온다.
         *  최종적으로   - 격년, 매1월, 매주 일요일 8:30:00 AM과  9:30:00 AM 
         *  
         *  *또 다른 rrule을 해석해 보면
         *  FREQ=DAILY;COUNT=10;INTERVAL=2
         *  ->격일로 하루에 10번 반복된다는 의미
         *  
         *  *또 다른 rrule을 해석해 보면
         *  FREQ=YEARLY;BYWEEKNO=4,8,12,16,20,24,28,32,36,40,44,48,52;BYDAY=TUE;
         *  ->4주째 화요일
         *  
         *  *또 다른 rrule를 해석해 보면
         *  FREQ=WEEKLY;BYDAY=TUE;BYHOUR=6;BYMINUTE=24;
         *  ->매주 화요일 아침 6:24
         *  
         ...
         *   FREQ=MONTHLY;BYDAY=MO,TU,WE,TH,FR;BYSETPOS=-1 --> "매월 마지막 주 월~금" 라는 의미.
		 *   
         * Strategy:
        ...
        @Override public int parsePart(String value, EventRecurrence er) {
            if (VALIDATE_UNTIL) {
                try {
                    ...
                    Calendar until = GregorianCalendar.getInstance();//Time until = new Time();
                    until.setTimeInMillis(ETime.parse(until.getTimeZone(), value));//until.parse(value);
