...
public class CalendarUtils {
    ...
    public static class TimeZoneUtils {
        ...
        private volatile static String mHomeTZ = ETime.getCurrentTimezone();
            ...
            protected void onQueryComplete(int token, Object cookie, Cursor cursor) {
                synchronized (mTZCallbacks) {
                    ...
                    while(cursor.moveToNext()) {
                        ...
                        // KEY_TIMEZONE_TYPE : They key for updating the use of auto/home time zones in Calendar
                        if (TextUtils.equals(key, CalendarCache.KEY_TIMEZONE_TYPE)) {
                        	
                        	// TIMEZONE_TYPE_AUTO : The value to write to KEY_TIMEZONE_TYPE if the provider should stay in sync with the device's time zone
                        	// TIMEZONE_TYPE_HOME : The value to write to KEY_TIMEZONE_TYPE if the provider should use a fixed time zone set by the user.

                        	// !op1 : op1이 true이면 false를, false이면 true를 
                        ...
                        } //KEY_TIMEZONE_INSTANCES_PREVIOUS : The key for reading the last time zone set by the user
                        else if (TextUtils.equals(key, CalendarCache.KEY_TIMEZONE_INSTANCES_PREVIOUS)) {
		...
		public String formatDateRange(Context context, long startMillis,
            ...
            synchronized (mSB) {
                mSB.setLength(0);           
                //date = ETime.formatDateRange(context, mF, startMillis, endMillis, flags, ETime.TIMEZONE_UTC).toString(); 
                date = ETime.formatDateTimeRange(context, mF, startMillis, endMillis, flags, tz).toString();              
            }
            return date;
        }
         ...
         */
		// 주의할 점? : 2nd para는 time zone id 또는
		//           "auto" 라는 텍스트가 될 수 있음
        ...
        public String getTimeZone(Context context, Runnable callback) {
            synchronized (mTZCallbacks){
                ...
                if (mFirstTZRequest) {
                	if (INFO) Log.i(TAG, "************************************");
                	if (INFO) Log.i(TAG, "getTimeZone : First TimeZone Request");
                	if (INFO) Log.i(TAG, "************************************");
                	
                    SharedPreferences prefs = getSharedPreferences(context, mPrefsName);
                    mUseHomeTZ = prefs.getBoolean(KEY_HOME_TZ_ENABLED, false);
                    
                    mHomeTZ = prefs.getString(KEY_HOME_TZ, ETime.getCurrentTimezone());
                    
                    // Only check content resolver if we have a looper to attach to use
                    Looper testObj = Looper.myLooper();
                    Thread whoAreYou = testObj.getThread();
            ...
            return mUseHomeTZ ? mHomeTZ : ETime.getCurrentTimezone();
        }
