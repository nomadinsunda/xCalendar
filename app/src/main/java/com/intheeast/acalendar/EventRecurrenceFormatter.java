    ...
    public static String getRepeatString(Context context, Resources r, EventRecurrence recurrence,
        ...
        if (includeEndString) {
            ...
            if (recurrence.until != null) {
                try {
                    Calendar t = GregorianCalendar.getInstance();//new Time();
                    t.setTimeInMillis(ETime.parse(t.getTimeZone(), recurrence.until));//t.parse(recurrence.until);
                    final String dateStr = DateUtils.formatDateTime(context,
                            t.getTimeInMillis(), DateUtils.FORMAT_NUMERIC_DATE);
        ...
        switch (recurrence.freq) {
            ...
            case EventRecurrence.WEEKLY: {
                if (recurrence.repeatsOnEveryWeekDay()) {
                    ...
                    if (recurrence.bydayCount > 0) {
                        ...
                        int day = EventRecurrence.calendarDay2Day(recurrence.startDate.get(Calendar.DAY_OF_WEEK));
            ...
            case EventRecurrence.MONTHLY: {
                if (recurrence.bydayCount == 1) {
                    int weekday = recurrence.startDate.get(Calendar.DAY_OF_WEEK);
                    // Cache this stuff so we won't have to redo work again later.
                    cacheMonthRepeatStrings(r, weekday);
                    int dayNumber = (recurrence.startDate.get(Calendar.DAY_OF_MONTH) - 1) / 7;
                    StringBuilder sb = new StringBuilder();
