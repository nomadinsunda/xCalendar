...
public class Event implements Cloneable {
    ...
    private final static String INSTANCES_SORT_ORDER = Instances.START_DAY + ","
    ...
    private static final String EVENTS_WHERE_TEST = Calendars.VISIBLE + "=1";
    private static final String SORT_EVENTS_BY_TEST = 
            "startDay, startMinute, title";
    ...
    //public static final String  BOTHENDS_EVENT_PROJECTION_NONE_FUNC = "NONE USE FUNC";
    public static final String  FOREMOST_EVENT_PROJECTION_MIN_FUNC = "min(startDay) as min_startDay";
    public static final String  BACKMOST_EVENT_PROJECTION_MAX_FUNC = "max(startDay) as max_startDay";
    
    // The projection to use when querying instances to build a list of events
    public static String[] FOREMOST_EVENT_PROJECTION = new String[] {
    	    FOREMOST_EVENT_PROJECTION_MIN_FUNC, // 0
    	    Instances._ID,                   // 1
    	    ...
    	    Instances.END_DAY,               // 7
    	    Instances.TITLE,                 // 8
            Instances.EVENT_LOCATION,        // 9
            Instances.ALL_DAY,               // 10  
            Instances.ALL_DAY + "=1 OR (" + Instances.END + "-" + Instances.BEGIN + ")>="
            + DateUtils.DAY_IN_MILLIS + " AS " + DISPLAY_AS_ALLDAY, // 11
    };
    
    public static String[] BACKMOST_EVENT_PROJECTION = new String[] {
    	BACKMOST_EVENT_PROJECTION_MAX_FUNC, // 0
	    Instances._ID,                   // 1
	    Instances.EVENT_ID,              // 2   
	    Instances.EVENT_TIMEZONE,        // 3
	    Instances.BEGIN,                 // 4
    ...
    // The indices for the projection array above.
    private static final int BOTHENDS_EVENT_PROJECTION_EVENT_ID_INDEX = 2;
    private static final int BOTHENDS_EVENT_PROJECTION_TIMEZONE_INDEX = 3;
    private static final int BOTHENDS_EVENT_PROJECTION_BEGIN_INDEX = 4;
    private static final int BOTHENDS_EVENT_PROJECTION_END_INDEX = 5;    
    private static final int BOTHENDS_EVENT_PROJECTION_START_DAY_INDEX = 6;
    private static final int BOTHENDS_EVENT_PROJECTION_END_DAY_INDEX = 7;
    private static final int BOTHENDS_EVENT_PROJECTION_TITLE_INDEX = 8;
    private static final int BOTHENDS_EVENT_PROJECTION_LOCATION_INDEX = 9;
    private static final int BOTHENDS_EVENT_PROJECTION_ALL_DAY_INDEX = 10;
    private static final int BOTHENDS_EVENT_PROJECTION_DISPLAY_AS_ALLDAY = 11;
    ...
    public static void loadEvents(Context context, ArrayList<Event> events, int startDay, int days,
        ...
        try {
            ...
            boolean hideDeclined = prefs.getBoolean(SettingsFragment.KEY_HIDE_DECLINED,
                    false);
            ...
            cEvents = instancesQuery(context.getContentResolver(), EVENT_PROJECTION, startDay,
                    endDay, where, null, SORT_EVENTS_BY /*INSTANCES_SORT_ORDER*/);
            ...
            //if (requestId != sequenceNumber.get()) {
            	//Log.i("tag", "Event.loadEvents : requestId != sequenceNumber.get(), so return");
                //return;
            //}

            // all day가 아닌 event들과 all day인 event들을 따로 연속적으로 묶는다
            // :all day event들을 먼저 리스트에 삽입한다
    ...
    /*
    public static void loadEventsTest(Context context, ArrayList<Event> events, int startDay, int days,
            int requestId, AtomicInteger sequenceNumber) {
        ...
        try {
            ...
            // You can specify additional display options for the final query results from a SQL SELECT statement.
            String where = EVENTS_WHERE_TEST;
            ...
            cEvents = instancesQueryForText(context.getContentResolver(), EVENT_PROJECTION, startDay,
                    endDay, where, null, SORT_EVENTS_BY_TEST);
    ...
    */
    ...
    public static void loadBothEndEvent(Context context, int loaderType, Event event, int startDay, int endDay,
            int requestId, AtomicInteger sequenceNumber) {

    	Cursor cEvent = null;
    	    	
        if (PROFILE) {
            Debug.startMethodTracing("loadEndEvents");
        }
                
        try {            
            if (loaderType == BothEndsEventLoader.FOREMOST_EVENT_LOADER) {
            	//BOTHENDS_EVENT_PROJECTION[0] = FOREMOST_EVENT_PROJECTION_MIN_FUNC;
            	cEvent = instancesQuery(context.getContentResolver(), FOREMOST_EVENT_PROJECTION, startDay,
	                    endDay, null, null, null);
	            	            
            }
            else {
            	//BOTHENDS_EVENT_PROJECTION[0] = BACKMOST_EVENT_PROJECTION_MAX_FUNC;
            	cEvent = instancesQuery(context.getContentResolver(), BACKMOST_EVENT_PROJECTION, startDay,
	                    endDay, null, null, null);
            }     
            ...
            buildBothEndEventFromCursor(event, cEvent, context, startDay, endDay);               
    ...
    /*
    private static final Cursor instancesQueryForText(ContentResolver cr, String[] projection,
            int startDay, int endDay, String selection, String[] selectionArgs, String orderBy) {
        
    	Calendar startTime = GregorianCalendar.getInstance();
    	long startDayMillis = ETime.getMillisFromJulianDay(startDay, startTime.getTimeZone());
    	startTime.setTimeInMillis(startDayMillis);//startTime.setJulianDay(startDay);
        //long start = startTime.toMillis(true);
        
    	Calendar endTime = GregorianCalendar.getInstance();//Time endTime = new Time();
    	long endDayMillis = ETime.getMillisFromJulianDay(endDay + 1, startTime.getTimeZone());
    	endTime.setTimeInMillis(endDayMillis);
        //endTime.setJulianDay(endDay + 1); // +1을 하는 이유는 endDay 쥴리안 데이로 endTime을 설정하면 yyyymmdd/00:00:00으로 설정되기 때문에
                                          // endDay가 가리키는 날짜의 event들이 쿼리되지 않는다. 그러므로 하루를 더 해줘야 함
        //long end = endTime.toMillis(true);
        
        // CONTENT_URI의 path segment는 millis를 적용해야 한다
        Uri.Builder builder = Instances.CONTENT_URI.buildUpon();
        ContentUris.appendId(builder, startDayMillis);
        ContentUris.appendId(builder, endDayMillis);
        
        return cr.query(builder.build(), projection, selection, selectionArgs, orderBy);
    }
	*/
    ...
    public static void buildBothEndEventFromCursor(
    		Event event, Cursor cEvent, Context context, int startDay, int endDay) {
    	
        if (cEvent == null) {
            Log.e(TAG, "buildBothEndEventFromCursor: null cursor!");
            return ;
        }

        int count = cEvent.getCount();

        if (count == 0) {
            return ;
        }        
        
    	cEvent.moveToPosition(-1);
        // Get info from Cursor
        //while (cEvent.moveToNext()) {       
        if (cEvent.moveToNext()) {	
        	event.id = cEvent.getLong(BOTHENDS_EVENT_PROJECTION_EVENT_ID_INDEX);            	
        	event.startMillis = cEvent.getLong(BOTHENDS_EVENT_PROJECTION_BEGIN_INDEX);
        	event.endMillis = cEvent.getLong(BOTHENDS_EVENT_PROJECTION_END_INDEX);
        	event.startDay = cEvent.getInt(BOTHENDS_EVENT_PROJECTION_START_DAY_INDEX);
        	event.endDay = cEvent.getInt(BOTHENDS_EVENT_PROJECTION_END_DAY_INDEX);                    
                    
        	event.title = cEvent.getString(BOTHENDS_EVENT_PROJECTION_TITLE_INDEX);
        	event.location = cEvent.getString(BOTHENDS_EVENT_PROJECTION_LOCATION_INDEX);
        	event.allDay = cEvent.getInt(BOTHENDS_EVENT_PROJECTION_ALL_DAY_INDEX) != 0;
        }
     ...
     * Each event is displayed as a non-overlapping rectangle.  
     * For normal events, these rectangles are displayed in separate columns in the week view and day view.  
     * For all-day events, these rectangles are displayed in separate rows along the top.  
     * In both cases, each event is assigned two numbers: N, and Max, 
     * that specify that this event is the Nth event of Max number of events that are displayed in a group. 
     * The width and position of each rectangle depend on the maximum number of rectangles that occur at the same time.
     *
    ...
    // 문제점은
    // 2015/09/22일 event들을 확인하기 바람
    // am2 ~ am5:30 헐 이벤트로 인해
    // am5가 start인 이벤트들의 width가 여전히
    // am2가 start인 이벤트들의 maxCols에 영향을 받음
    // am2 ~ am5:30 헐 이벤트로 인해
    // am2시 이벤트 이후에 activeList.isEmpty()가 발생되지 않아
    // 여전히 am5가 start인 이벤트들의 width는 
    // am2시 start인 이벤트들의 width와 동일함...
    private static void doComputePositions(ArrayList<Event> eventsList,
        ...
        for (Event event : eventsList) {
           ...
           if (!doAlldayEvents) {
            ...
            // Empty columns are represented by
    ...
    //private static final String SORT_EVENTS_BY =
            //"begin ASC, end DESC, title ASC";
    // DayView.reloadEvents에서 쿼리하는 이벤트들의 정렬순은 위와 같다
    // ASC : 값이 작은 쪽에서부터 큰 쪽으로의 순서를 말하며, 역방향의 경우는 내림차순(descending order)이다
    private static long removeNonAlldayActiveEvents(
        ...
        // An event on the active list becomes inactive 
        // when its end time is less than or equal to the current event's start time.
        while (iter.hasNext()) {
            final Event active = iter.next();
            ...
            if ((active.getStartMillis() + duration) <= start) {///////////////////////////////////////////////////////////////
            	// inactive event가 되면서,
            	// 해당 inactive event가 colMask에 할당?받은 비트 포지션이 삭제[0으로]된다
                colMask &= ~(1L << active.getColumn());
                // 삭제도 함
                iter.remove();
            }
