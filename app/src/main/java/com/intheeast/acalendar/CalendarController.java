...
import com.intheeast.selectcalendars.SelectCalendarsActivity;
import com.intheeast.settings.SettingsActivity;
...
public class CalendarController {
    ...
    public static final int MAX_CALENDAR_WEEK = 3497; // weeks between 1/1/1970 and 1/1/2037 : 1/1/2037의 week position 값은 3496인데 왜 3497라고 한 것은 
                                                      // 인덱싱 오더링이 0부터 시작하기 때문에 총 3497개의 week가 존재한다는 것을 의미한다
    */
    
    public static final int MIN_CALENDAR_YEAR = 1900;
    public static final int MAX_CALENDAR_YEAR = 2100;
    public static final int MIN_CALENDAR_WEEK = 0;
    public static final int MAX_CALENDAR_WEEK = 10437; // weeks between 1/1/1900 and 1/1/2100 : 1/1/2100의 week position 값은 10436 이다
    ...
    private final Calendar mTime = GregorianCalendar.getInstance();
    ...
    private final Runnable mUpdateTimezone = new Runnable() {
        ...
        public void run() {
            ...
            TimeZone timezone = TimeZone.getTimeZone(Utils.getTimeZone(mContext, this));
    ...
    public interface EventType {
        ...
        final long SETTINGS = 1L << 9;

        // User has pressed the home key
        final long USER_HOME = 1L << 10;

        // date range has changed, update the title
        final long UPDATE_TITLE = 1L << 11;

        // select which calendars to display
        final long LAUNCH_SELECT_VISIBLE_CALENDARS = 1L << 12;
                // 
        final long EXPAND_EVENT_LISTVIEW = 1L << 13;
        
        final long COLLAPSE_EVENT_VIEW = 1L << 14;
        
        final long EXPAND_EVENT_VIEW_OK = 1L << 15;
        
        final long EXPAND_EVENT_VIEW_NO_OK = 1L << 16;
        
        final long COLLAPSE_EVENT_VIEW_OK = 1L << 17;
        
        final long COLLAPSE_EVENT_VIEW_NO_OK = 1L << 18;
        
        final long GO_TO_MONTH_ANIM = 1L << 19;
        
        final long GO_TO_YEAR_ANIM = 1L << 20;
        
        final long SEARCH_LISTVIEW_EMPTY = 1L << 21;
        
        final long BOTHENDS_EVENTS_QUERY_COMPLETION = 1L << 22;
        
        final long GOT_ALL_BOTHENDS_EVENTS = 1L << 23;
        
        final long BOTHENDS_EVENTS_CHANGED = 1L << 24;
    ...
    public interface ViewType {
        ...
        final int EDIT = 6;
        final int SEARCH = 7;
        final int SETTINGS = 8;        
        //final int DAY_EVENT_INFO = 8;
        final int MAX_VALUE = 9;
    }
    
    public interface FragmentTag {
        final String DETAIL_TAG = "DETAIL TAG";
        final String CURRENT_TAG = "CURRENT TAG";
        final String AGENDA_TAG = "AGENDA TAG";
        final String DAY_TAG = "DAY TAG";
        final String WEEK_TAG = "WEEK TAG";
        final String MONTH_TAG = "MONTH TAG";
        final String YEAR_TAG = "YEAR TAG";
        final String EDIT_TAG = "EDIT TAG";
        final String SEARCH_TAG = "SEARCH TAG";
    }
    ...
    public static class EventInfo {
        ...
        public Calendar selectedTime; // the selected time in focus
    ...
    public static final long EXTRA_GOTO_BACK_TO_PREVIOUS_FORCE = 16;
    public static final long EXTRA_GOTO_DATE_NOT_UPDATE_SEC_ACTIONBAR = 32;
    
    public static final long EXTRA_BOTHENDS_FOREMOST_EVENT = 1;
    public static final long EXTRA_BOTHENDS_BACKMOST_EVENT = 2;
    
    public interface GOT_BOTHENDS_EVENT_TYPE {
    	final long GOT_FOREMOST_EVENT = 1L;       
        final long GOT_BACKMOST_EVENT = 1L << 1;
        final long GOT_ALL_BOTHENDS_EVENT = GOT_FOREMOST_EVENT | GOT_BACKMOST_EVENT;
    }
    ...
    private CalendarController(Context context) {
        ...
        mDetailViewType = Utils.getSharedPreference(mContext,
        		SettingsFragment.KEY_DETAILED_VIEW,
    ...
    public void sendEventRelatedEventWithExtraWithTitleWithCalendarId(Object sender, long eventType,
        ...
        /*
        if (eventType == EventType.CREATE_EVENT || eventType == EventType.EDIT_EVENT) {
        	info.viewType = ViewType.EDIT;
        }
        else if (eventType == EventType.VIEW_EVENT_DETAILS) {
        	info.viewType = ViewType.CURRENT;
        }
		*/
        
        info.id = eventId;
        info.startTime = GregorianCalendar.getInstance(TimeZone.getTimeZone(Utils.getTimeZone(mContext, mUpdateTimezone)));
        info.startTime.setTimeInMillis(startMillis);
        if (selectedMillis != -1) {
            info.selectedTime = GregorianCalendar.getInstance(TimeZone.getTimeZone(Utils.getTimeZone(mContext, mUpdateTimezone)));
            info.selectedTime.setTimeInMillis(selectedMillis);
        } else {
            info.selectedTime = info.startTime;
        }
        info.endTime = GregorianCalendar.getInstance(TimeZone.getTimeZone(Utils.getTimeZone(mContext, mUpdateTimezone)));
        info.endTime.setTimeInMillis(endMillis);
        info.x = x;
        info.y = y;
     ...
     */
    public void sendEvent(Object sender, long eventType, Calendar start, Calendar end, long eventId,
            int viewType) {
    ...
    public void sendEvent(Object sender, long eventType, Calendar start, Calendar end, Calendar selected,
    ...
    public void sendEvent(Object sender, final EventInfo event) {
        ...
        if (event.viewType == ViewType.DETAIL) {
        	...
        	// setViewType 이외에서 설정되는 실질적인 코드
            mViewType = event.viewType; ///////////////////////////////////////////////

            // Utils.getAllowWeekForDetailView()는 영원히 false를 리턴한다
            // ECalendarMainActivity::onCreate에서 다음과 같이 호출하기 때문이다
            // mIsMultipane는 false로 설정됨
        ...
        if (event.startTime != null) {
            startMillis = event.startTime.getTimeInMillis();
        }

        // Set mTime if selectedTime is set
        if (event.selectedTime != null && event.selectedTime.getTimeInMillis() != 0) {
            mTime.setTimeInMillis(event.selectedTime.getTimeInMillis());/////////////////////////////////////////////////////////////////////////////
        } else {
            if (startMillis != 0) {
                // selectedTime is not set so set mTime to startTime if it is not
                // within start and end times
                long mtimeMillis = mTime.getTimeInMillis();
                if (mtimeMillis < startMillis
                        || (event.endTime != null && mtimeMillis > event.endTime.getTimeInMillis())) {
        ...
        synchronized (this) {
            ...
            if (mFirstEventHandler != null) {
                ...
                EventHandler firstEventHandler = mFirstEventHandler.second;
                if (firstEventHandler != null && (firstEventHandler.getSupportedEventTypes() & event.eventType) != 0
        ...
        if (!handled) {
            ...
            if (event.eventType == EventType.CREATE_EVENT) {
                launchCreateEvent(event.startTime.getTimeInMillis(), endTime,
                        event.extraLong == EXTRA_CREATE_ALL_DAY, event.eventTitle,
                        event.calendarId);
                return;
            } else if (event.eventType == EventType.VIEW_EVENT) {
                launchViewEvent(event.id, event.startTime.getTimeInMillis(), endTime,
                        event.getResponse());
                return;
            } else if (event.eventType == EventType.EDIT_EVENT) {
                launchEditEvent(event.id, event.startTime.getTimeInMillis(), endTime, true);
                return;
            } else if (event.eventType == EventType.VIEW_EVENT_DETAILS) {
                launchEditEvent(event.id, event.startTime.getTimeInMillis(), endTime, false);
                return;
            } else if (event.eventType == EventType.DELETE_EVENT) {
                launchDeleteEvent(event.id, event.startTime.getTimeInMillis(), endTime);
                return;
            } else if (event.eventType == EventType.SEARCH) {
                launchSearch(event.id, event.query, event.componentName);
                return;
            }
            else if (event.eventType == EventType.SETTINGS) {
                launchSetting(event.id, event.query, event.componentName);
                return;
            }
        }
    }
    ...
    public void setTime(long millisTime) {
        mTime.setTimeInMillis(millisTime);
    ...
    private void launchSelectVisibleCalendars() {
        ...
        //activity.overridePendingTransition(R.layout.activity_slide_up, R.layout.activity_slide_up_background);
    }
    ...
    private void launchCreateEvent(long startMillis, long endMillis, boolean allDayEvent,
        ...
        mEventId = -1;
        mContext.startActivity(intent);
        // 현재 mContext는 ECalendarApplication이므로 Activity로 casting이 되지 않는다
        // 런타임 에러를 발생함...
        //Activity activity = (Activity) mContext;        
        //activity.overridePendingTransition(R.layout.activity_slide_up, R.layout.activity_slide_up_background);
    }
    ...
    public void launchViewEvent(long eventId, long startMillis, long endMillis, int response) {
        ...
        intent.setClass(mContext, CentralActivity.class);
        intent.putExtra(EXTRA_EVENT_BEGIN_TIME, startMillis);
        intent.putExtra(EXTRA_EVENT_END_TIME, endMillis);
...
//    }
    ...
    private void launchSetting(long eventId, String query, ComponentName componentName) {
    	Intent intent = new Intent(Intent.ACTION_VIEW);
    ...
    // initFragments에서 viewType == ViewType.EDIT인 경우 호출된다...
    ...
    private String eventInfoToString(EventInfo eventInfo) {
        ...
        if ((eventInfo.eventType & EventType.GO_TO) != 0) {
            ...
            tmp = "Search";
        } else if ((eventInfo.eventType & EventType.SETTINGS) != 0) {
            tmp = "Settings";
