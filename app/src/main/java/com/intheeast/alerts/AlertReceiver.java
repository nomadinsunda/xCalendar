...
import com.intheeast.alerts.AlertService.NotificationWrapper;
 ...
 *    -n "com.intheeast.ecalendar/.alerts.AlertReceiver"
 */
public class AlertReceiver extends BroadcastReceiver {
    private static final String TAG = "AlertReceiver";
    static final boolean INFO = true;

    public static final String MAP_ACTION = "com.intheeast.ecalendar.MAP";
    private static final String CALL_ACTION = "com.intheeast.ecalendar.CALL";
    private static final String MAIL_ACTION = "com.intheeast.ecalendar.MAIL";
    ...
    public static final String EVENT_REMINDER_APP_ACTION =
            "com.intheeast.ecalendar.EVENT_REMINDER_APP";
    ...
    public void onReceive(final Context context, final Intent intent) {
        if (AlertService.INFO) {
            if (INFO) Log.i(TAG, "onReceive: action=" + intent.getAction());
            if (INFO) Log.i(TAG, "*******************************************************");
            if (INFO) Log.i(TAG, intent.toString());
            if (INFO) Log.i(TAG, "*******************************************************");
        }
        ...
        if (MAP_ACTION.equals(intent.getAction())) {
            ...
            if (intent.getAction().equalsIgnoreCase(CalendarContract.ACTION_EVENT_REMINDER)) {
            	if (uri != null) {
            		String alarmTimeStr = uri.getLastPathSegment();/////////////////////////////////////////////////////////////
            		long alarmMillis = Long.parseLong(alarmTimeStr);
            		// Time.getCurrentTimezone() : Returns the timezone string that is currently set for the device.
            		TimeZone timezone = TimeZone.getTimeZone(ETime.getCurrentTimezone());
            		Calendar alarmTime = GregorianCalendar.getInstance(timezone);//new Time(Time.getCurrentTimezone());            		
            ...
            //beginStartingService(getApplicationContext(), i);
        }
    }
    ...
    */
    ...
    private static PendingIntent createAloneAlertActivityIntent(Context context, long alarmTime) {
    	if (INFO) Log.i(TAG, "createAloneAlertActivityIntent");
    	
        Intent clickIntent = new Intent();
        clickIntent.setClass(context, AloneAlertActivity.class);
        clickIntent.addFlags(Intent.FLAG_ACTIVITY_NEW_TASK);
        clickIntent.putExtra("alarmTime", alarmTime);
        return PendingIntent.getActivity(context, 0, clickIntent,
                    PendingIntent.FLAG_ONE_SHOT | PendingIntent.FLAG_UPDATE_CURRENT);
    }
    ...
    private static Notification buildBasicNotification(Notification.Builder notificationBuilder,
            Context context, String title, String summaryText, long alarmTime, long startMillis, long endMillis,
            long eventId, int notificationId, boolean doPopup, boolean addActionButtons) {
    	
    	if (INFO) Log.i(TAG, "buildBasicNotification");
        ...
        notificationBuilder.setDeleteIntent(deleteIntent);
        if (doPopup) {
        	if (INFO) Log.i(TAG, "buildBasicNotification : DO POP UP!!!");
        	
        	// An intent to launch instead of posting the notification to the status bar
        	// :���� notification bar�� alarm�� ���� �ʴ´�...
        	//  ���� user�� ���Ű�� �����ų� ȨŰ�� ������ popup view�� ������ٸ�,,,
        	//  ->notification bar�� �ٽ� ��ϵǾ�� ���� ������???
        	//  �����̳� ��ȣ�� �ɷ� ������ ��� �Ǵ°�?
        	PowerManager pm = (PowerManager) context.getSystemService(Context.POWER_SERVICE);
        	if (pm.isScreenOn()) {
        		if (INFO) Log.i(TAG, "buildBasicNotification : ScreenOn");
        		
        		KeyguardManager km = (KeyguardManager) context.getSystemService(Context.KEYGUARD_SERVICE);
        		if (!km.inKeyguardRestrictedInputMode()) {
        			if (INFO) Log.i(TAG, "buildBasicNotification : NO keyguard restricted input mode");
        			notificationBuilder.setFullScreenIntent(createAloneAlertActivityIntent(context, alarmTime), true);
        		}
        	}
        	else {
        		// screen�� on ���Ѿ� �ϴ°�? 
        ...
        if (Utils.isJellybeanOrLater()) {
            ...
            // A higher priority will encourage notification manager to expand it.
            // � �̿� ������
            // BigTextStyle ����Ҷ� bigText�� ������ 2���̻� �Ѿ�� ó������ 2���̻� ���� �����ֱ� ���ؼ���
            // PRIORITY_MAX�� �����ؾ� �Ѵٴ� ������ ����...
    ...
    public static NotificationWrapper makeExpandingNotification(Context context, String title,
            String summaryText, String description, long alarmTime, long startMillis, long endMillis, long eventId,
            int notificationId, boolean doPopup) {
    	
    	if (INFO) Log.i(TAG, "makeExpandingNotification");
    	
        Notification.Builder basicBuilder = new Notification.Builder(context);
        Notification notification = buildBasicNotification(basicBuilder, context, title,
                summaryText, alarmTime, startMillis, endMillis, eventId, notificationId, doPopup, true);
        if (Utils.isJellybeanOrLater()) {
            // Create a new-style expanded notification
            Notification.BigTextStyle expandedBuilder = new Notification.BigTextStyle();
            if (description != null) {
