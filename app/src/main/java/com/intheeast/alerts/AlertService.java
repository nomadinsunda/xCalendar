...
public class AlertService extends Service {
	...
	*/    
    ...
    void processMessage(Message msg) {
    	if (INFO) Log.i(TAG, "processMessage");
        ...
        long alarmTimeMillis = bundle.getLong(android.provider.CalendarContract.CalendarAlerts.ALARM_TIME);
        
        if (INFO) {
        	//Time alarmTime = new Time(Time.getCurrentTimezone());  
        	TimeZone timezone = TimeZone.getTimeZone(ETime.getCurrentTimezone());
            Calendar alarmTime = GregorianCalendar.getInstance(timezone);
            
    		alarmTime.setTimeInMillis(alarmTimeMillis); 
    		//if (INFO) Log.i(TAG, alarmTime.format3339(false));
            //Log.i(TAG, "processMessage:ALARM_TIME=" + String.valueOf(alarmTimeMillis));
            Log.i(TAG, "processMessage:Action=" + action);                    
        }
        ...
        if (providerReminder) {
            ...
            if (!sReceivedProviderReminderBroadcast) {
                ...
                Log.i(TAG, "Setting key " + PROVIDER_REMINDER_PREF_KEY + " to: true");
        ...
        if (providerReminder ||
            ...
            //updateAlertNotification(this, alarmTimeMillis);
            updateAlertNotification(getApplicationContext(), alarmTimeMillis);
        ...
        if (sReceivedProviderReminderBroadcast == null || !sReceivedProviderReminderBroadcast) {
            Log.i(TAG, "Scheduling next alarm with AlarmScheduler. "
    ...
    static boolean updateAlertNotification(Context context) {
        ...
        if (INFO) {
            Log.i(TAG, "Beginning updateAlertNotification");
        }

        if (!prefs.getBoolean(SettingsFragment.KEY_ALERTS, true)) {
            if (INFO) {
        ...
        if (alertCursor == null || alertCursor.getCount() == 0) {
            ...
            if (INFO) Log.i(TAG, "No fired or scheduled alerts");
            nm.cancelAll();
            return false;
        }
    ...
    static boolean updateAlertNotification(Context context, long alarmTime) {
    	if (INFO) Log.i(TAG, "updateAlertNotification");
        ...
        if (!prefs.getBoolean(SettingsFragment.KEY_ALERTS, true)) {
            if (INFO) Log.i(TAG, "alert preference is OFF");
        ...
        ///////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
        Cursor alertCursor = cr.query(CalendarAlerts.CONTENT_URI, ALERT_PROJECTION,
                (ACTIVE_ALERTS_SELECTION + alarmTime), ACTIVE_ALERTS_SELECTION_ARGS,
                ACTIVE_ALERTS_SORT);
        ...
        return generateAlerts(context, nm, AlertUtils.createAlarmManager(context), prefs,
                alertCursor, alarmTime, currentTime, MAX_NOTIFICATIONS);
    }
    ...
    public static boolean generateAlerts(Context context, NotificationMgr nm,
        ...
        if (INFO) {
            Log.i(TAG, "alertCursor count:" + alertCursor.getCount());
        }
        ...
        if (numLowPriority > 0) {
            ...
            if (INFO) {
              Log.i(TAG, "Quietly posting digest alarm notification, numEvents:" + numLowPriority
        ...
        if (currentNotificationId <= maxNotifications) {
            ...
            if (INFO) {
                Log.i(TAG, "Canceling leftover notification IDs " + currentNotificationId + "-"
                        + maxNotifications);
            }
        }
        ...
        if (nextRefreshTime < Long.MAX_VALUE && nextRefreshTime > currentTime) {
            ...
            if (INFO) {
                ...
                time.setTimeInMillis(nextRefreshTime);
    ...
    public static boolean generateAlerts(Context context, NotificationMgr nm,
            AlarmManagerInterface alarmMgr, SharedPreferences prefs, Cursor alertCursor,
            final long alarmTime, final long currentTime, final int maxNotifications) {
    	
    	if (INFO) Log.i(TAG, "generateAlerts:alertCursor count=" + alertCursor.getCount());
        
        // Process the query results and bucketize events.
        ArrayList<NotificationInfo> Events = new ArrayList<NotificationInfo>();        
        
        int numFired = processQuery(alertCursor, context, alarmTime, Events);
        ...
        // iphone������ notification bar�� Ư�� notification�� ������ Ȯ������ ���� ���
        // ����� ������ ���� �︮�� ���� �� �� �ִ�...
        // �׷� �뵵�� ����� ����
        long nextRefreshTime = Long.MAX_VALUE;
        int currentNotificationId = 1;
        ...
        for (int i = 0; i < Events.size(); i++) {
            ...
            String summaryText = AlertUtils.formatTimeLocation(context, info.startMillis,
                    info.allDay, info.location);
            /////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
            postNotification(info, summaryText, context, notificationPrefs, nm,
                    currentNotificationId++);
    ...
    private static void logEventIdsBumped(List<NotificationInfo> list1,
        ...
        if (ids.length() > 0) {
            Log.i(TAG, "Reached max postings, bumping event IDs {" + ids.toString()
                    + "} to digest.");
        }
    }
        ...
        try {
            while (alertCursor.moveToNext()) {
                ...
                */
                ...
                // �Ʒ� �ּ����� multiple installed calendar apps��� �� ������ android calendar�� ���ϴ� ��?
                // �ƴϸ� Ÿ Calendar app���� ���ϴ°�?
                // :???
                //  ->���� ������ ��Ȳ�� ���� �ڵ���� ����,
                //    �ش� �̺�Ʈ�� stata�� CalendarAlerts.STATE_SCHEDULED�̸� 
                //    CalendarAlerts.STATE_FIRED�� �ش� row�� CalendarAlerts.STATE �ʵ带 ������Ʈ �Ѵ�
                ...
                if (AlertUtils.BYPASS_DB && ((currentTime - alarmTime) / MINUTE_MS < 1)) {
                	...
                	//  10����,... 
                	//  �̷��ϴٴ� ���̴�
                	// *BYPASS_DB�� �ǹ̴� �ش� row�� CalendarAlerts.STATE �ʵ带 ������Ʈ ���� �ʰڴٴ� �ǹ̰� �ƴұ�???
                	//  :�̹� Ÿ calendar app���� �ʵ带 ������Ʈ�Ͽ��ٸ� ���̴�
                	// *(currentTime - alarmTime) / MINUTE_MS < 1 ��Ȳ��?
                	//  :alarmTime���� ����� �ð��� 1�й̸��̶�� ���� �� ������...
                	
                	// *�Ƹ��� ������ alarmTime�� ������ ������ event���� re-fire notification�� ���� ���� ���� �ƴұ�???
                	//  :�ƴ� ���� �ִ�
				...
				*/
                ...
                if (sendAlert) {
                	// CalendarAlerts.STATE_SCHEDULED : An alert begins in this state when it is first created.
                ...
                // grace period : ���� �Ⱓ(start time���� end time �Ⱓ�� ����)
                ...
                // �� �̷� ���� �Ѥ�;
                // :������ select ������ ������ ���� �߱� �����̴�
                //  (state=STATE_FIRED OR state=STATE_SCHEDULED) AND alarmTime<=currentTime 
                //  ���� ��� AlertActivity�� popup �Ǿ��� �� �׳� �����ϸ�
                //  �ش� event�� FIRED�� ��Ȳ�� �����ȴ�
                //  �׷��ٸ�...???
                //  �츮�� ��� �ؾ� �� ���ΰ�???
                //  state=STATE_SCHEDULED AND alarmTime<=currentTime : �ð��븦 �����ϴ� ���� ���� ��Ȳ�̴� currentTime�̶�� ���� 
                //                                                     updateAlertNotification �Լ����� System.currentTimeMillis()�� ������ ���̱� �����̴�
                //  state=STATE_SCHEDULED�� controll ����
                //  *** �ƴϴ� ���� alarm�� �����ϴ� ���� �ּ� ������ Minute �� ���̴�? : �� �׷� ��Ȳ�� �߻��� �� �� �ִ°�???
                //  *�����?
                //  select ������ state=STATE_SCHEDULED AND alarmTime<=currentTime �� �ؼ�
                //  �켱������ �����Ѵ�...���⼭ �� �Ѱ��� event�� user���� ���� �ش�
                //  :�׷��� ���� ����̽����� �� �Ѱ��� Ķ������ �������� �ʾҴ� ��Ȳ���� eCalendar�� ��ġ�Ǿ��ٸ�,,,
                //   ������ state=STATE_SCHEDULED �̺�Ʈ���� �������ٵ�...�Ѥ�;
                //   ���� ��ġ�� Ķ�������� �̺�Ʈ �˸� ����� ��Ȱ��ȭ�Ǿ� �ִٸ�...
                //   ->�ϴ� �̰� ��Ȯ�� ������ �� �� ���� �˸� ����� ��Ȱ���Ǿ� �ִٰ��ؼ� state==STATE_FIRED�� �������� �ʴ� �ٴ� ���� ����
                
                
                
                // (state=STATE_FIRED OR state=STATE_SCHEDULED) AND alarmTime<=currentTime �̱� ������ ����� �����ϴ�
                // :FIRED->DISMISS�� ���°� ������ �ʾҴٴ� ���̴�
                ...
                if (eventIds.containsKey(eventId)) {
                    ...
                    // ������ �̺�Ʈ��� oldBeginTimeAdjustedForAllDay�� beginTimeAdjustedForAllDay�� ������ ���� �ƴѰ�???
                    // oldBeginTimeAdjustedForAllDay�� beginTimeAdjustedForAllDay �̶�� �� �P �� �ش� �̺�Ʈ�� beginTime�ε�...
                    ...
                    if (dropOld) {
                        ...
                        if (INFO) {
                            Log.i(TAG, "Dropping alert for recurring event ID:" + oldInfo.eventId
    ...
    static int processQuery(final Cursor alertCursor, final Context context,
            final long alarmTime, ArrayList<NotificationInfo> Events) {
    	...
    	try {
            while (alertCursor.moveToNext()) {
                ...
                // TODO: Prefer accepted events in case of ties.
                NotificationInfo newInfo = new NotificationInfo(eventName, location,
                        description, alarmTime, beginTime, endTime, eventId, allDay, true/*newAlert*/);
    ...
    private static void postNotification(NotificationInfo info, String summaryText,
        ...
        if (INFO) {
            Log.i(TAG, "Posting individual alarm notification, eventId:" + info.eventId
    ...
    private static void postNotification(NotificationInfo info, String summaryText,
            Context context, NotificationPrefs prefs,
            NotificationMgr notificationMgr, int notificationId) {
    	        
    	if (INFO) Log.i(TAG, "postNotification");
    	
        String tickerText = getTickerText(info.eventName, info.location);
        NotificationWrapper notification = AlertReceiver.makeExpandingNotification(context,
                info.eventName, summaryText, info.description, info.alarmMillis, info.startMillis,
    ...
    static class NotificationInfo {
        ...
        NotificationInfo(String eventName, String location, String description, long alarmMillis, long startMillis,
                long endMillis, long eventId, boolean allDay, boolean newAlert) {
            this.eventName = eventName;
            this.location = location;
            this.description = description;
    ...
    /* package */ static class NotificationPrefs {
        ...
        private boolean getDoPopup() {
        	/*
            if (doPopup < 0) {
                if (prefs.getBoolean(SettingsFragment.KEY_ALERTS_POPUP, false)) {
                	if (INFO) Log.i(TAG, "getDoPopup : doPopup=1");
                    doPopup = 1;
                } else {
                	if (INFO) Log.i(TAG, "getDoPopup : doPopup=0");
                    doPopup = 0;
                }
            }
            */
        	...
        	if (doPopup < 0) {
        		if (Utils.getSharedPreference(context, SettingsFragment.KEY_ALERTS_POPUP, false)) {
        			if (INFO) Log.i(TAG, "getDoPopup : doPopup=1");
                    doPopup = 1;
        		} else {
                	if (INFO) Log.i(TAG, "getDoPopup : doPopup=0");
        ...
        try {
            ...
            while (cursor.moveToNext()) {
                ...
                if (alarmTime != newAlarmTime) {
                    if (INFO) {
                        Log.i(TAG, "rescheduling missed alarm. alarmTime: " + newAlarmTime);
                    }
    ...
    private final class ServiceHandler extends Handler {
        ...
        public void handleMessage(Message msg) {
        	if (INFO) Log.i(TAG, "ServiceHandler");
            processMessage(msg);
    ...
    public int onStartCommand(Intent intent, int flags, int startId) {
    	if (INFO) Log.i(TAG, "onStartCommand");
        ...
        if (intent != null) {
            ...
            if (INFO) Log.i(TAG, "onStartCommand:sendMessage");
        }
