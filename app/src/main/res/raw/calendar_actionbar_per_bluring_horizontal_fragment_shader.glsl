...
void main(void)
{
	...
	//for (int i = 1; i < 3; i++) { // linear sampling
	    gl_FragColor += texture2D( u_Texture, vec2((gl_FragCoord.x + offset[i])/v_Width, (gl_FragCoord.y + 0.0)/v_Height) ) * weight[i];
		gl_FragColor += texture2D( u_Texture, vec2((gl_FragCoord.x - offset[i])/v_Width, (gl_FragCoord.y + 0.0)/v_Height) ) * weight[i];		
	}	
}